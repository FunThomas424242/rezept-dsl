 ...
 * hovers, is a proxy.
 */
public class RezeptHyperlinkDetector implements IHyperlinkDetector {
	 ...
	 */
	public RezeptHyperlinkDetector(Resource resource) {
		textResource = (com.github.funthomas424242.rezeptsammler.rezept.resource.rezept.IRezeptTextResource) resource;
	}
		...
		for (EObject eObject : elementsAtOffset) {
			if (eObject.eIsProxy()) {
				...
				if (resolvedEObject.eResource() != null) {
					IHyperlink hyperlink = new com.github.funthomas424242.rezeptsammler.rezept.resource.rezept.ui.RezeptHyperlink(new Region(offset, length), resolvedEObject, text);
