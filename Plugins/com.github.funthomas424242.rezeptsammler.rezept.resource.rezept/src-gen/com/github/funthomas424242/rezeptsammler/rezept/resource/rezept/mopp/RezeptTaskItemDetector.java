...
/**
 * The RezeptTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * RezeptTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class RezeptTaskItemDetector {
	...
	public List<com.github.funthomas424242.rezeptsammler.rezept.resource.rezept.mopp.RezeptTaskItem> findTaskItems(String text, int line, int charStart) {
		List<com.github.funthomas424242.rezeptsammler.rezept.resource.rezept.mopp.RezeptTaskItem> foundItems = new ArrayList<com.github.funthomas424242.rezeptsammler.rezept.resource.rezept.mopp.RezeptTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new com.github.funthomas424242.rezeptsammler.rezept.resource.rezept.mopp.RezeptTaskItem(keyword, message, localLine, offset, end));
