...
public class RezeptDefaultNameProvider implements com.github.funthomas424242.rezeptsammler.rezept.resource.rezept.IRezeptNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = com.github.funthomas424242.rezeptsammler.rezept.resource.rezept.util.RezeptEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
