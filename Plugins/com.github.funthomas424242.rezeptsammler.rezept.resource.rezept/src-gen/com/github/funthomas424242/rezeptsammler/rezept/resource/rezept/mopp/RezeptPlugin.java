 ...
 */
public class RezeptPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "com.github.funthomas424242.rezeptsammler.rezept.resource.rezept";
	/**
	...
	private static RezeptPlugin plugin;
	...
	public static RezeptPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, RezeptPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, RezeptPlugin.PLUGIN_ID, message);
		}
		final RezeptPlugin pluginInstance = RezeptPlugin.getDefault();
